      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card className="flex flex-col p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-50">Contacts</h2>
            <Link href="/crm/contacts" className="text-xs font-medium text-brand-600 dark:text-brand-400 hover:underline">
              View all →
            </Link>
          </div>
          <div className="mt-3 flex flex-col divide-y divide-purple-400/10">
            {contacts.length === 0 ? (
              <p className="py-2 text-xs text-slate-500 dark:text-slate-400">No contacts yet.</p>
            ) : (
              contacts.map((c) => (
                <div key={c.id} className="py-2">
                  <p className="text-sm font-medium text-slate-900 dark:text-slate-50">{c.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{[c.title, c.email, c.phone].filter(Boolean).join(" · ")}</p>
                </div>
              ))
            )}
          </div>
        ...
        <Card className="flex flex-col p-5">
          ...
          <div className="mt-3 flex flex-col divide-y divide-purple-400/10">
            {opportunities.length === 0 ? (
              <p className="py-2 text-xs text-slate-500 dark:text-slate-400">No opportunities yet.</p>
            ) : (
              opportunities.map((o) => (
                <Link
                  key={o.id}
        ...
        <Card className="flex flex-col p-5">
          ...
          <div className="mt-3 flex flex-col divide-y divide-purple-400/10">
            {sites.length === 0 ? (
              <p className="py-2 text-xs text-slate-500 dark:text-slate-400">No sites yet.</p>
            ) : (
              sites.map((s) => (
                <Link
                  key={s.id}
