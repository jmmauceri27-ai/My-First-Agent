    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-2">
        <Card className="border-t-4 p-5" style={{ borderTopColor: "#2a78d6" }}>
          ...
          <p className="mt-1.5 text-3xl font-extrabold tabular-nums text-zinc-900 dark:text-zinc-50">
            {datasets.length}
          </p>
        </Card>
      ...
      <section>
        <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Your datasets</h2>
        {datasets.length === 0 ? (
            ...
            <table className="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
              <thead className="bg-zinc-50 dark:bg-zinc-900">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-semibold text-zinc-500">Category</th>
                  <th className="px-4 py-2 text-left font-semibold text-zinc-500">Rows</th>
                  <th className="px-4 py-2 text-left font-semibold text-zinc-500">Uploaded</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                {datasets.map((d) => (
                  <tr key={d.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                    ...
                    <td className="px-4 py-2 tabular-nums text-zinc-600 dark:text-zinc-400">{d.rowCount}</td>
      ...
      <section>
        <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Your dashboards</h2>
        {dashboards.length === 0 ? (
          <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
