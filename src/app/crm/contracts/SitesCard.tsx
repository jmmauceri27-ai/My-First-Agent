...
import { bulkCreateSitesForContractAction, bulkDeleteSitesAction, deleteSiteAction, parseSiteSheetAction } from "../../network/actions";
...
export default function SitesCard({
  ...
  async function handleImport() {
    ...
    try {
      ...
      const result = await bulkCreateSitesForContractAction(contractId, companyId, rows);
      if (result.error) {
  ...
  async function handleBulkDelete() {
    ...
    if (!window.confirm(`Remove ${count} site${count === 1 ? "" : "s"} from this agreement? This can't be undone.`)) {
      return;
    }
  ...
  return (
    <Card className="flex flex-col p-5">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Upload the properties covered by this agreement — adds to Network → Sites, linked back to this agreement.
          </p>
        </div>
      </div>
      ...
      {sites.length === 0 && !parsedRows && (
        <p className="mt-4 text-xs text-slate-500 dark:text-slate-400">No sites uploaded yet for this agreement.</p>
      )}
    </Card>
  );
}
