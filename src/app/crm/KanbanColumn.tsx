  ...
  return (
    <div
      ...
      className={`flex min-w-[200px] flex-1 flex-col gap-2 rounded-xl border p-3 transition-colors ${
        isOver ? "border-brand-400 bg-brand-500/15" : "border-purple-200 bg-slate-50 dark:border-purple-500/20 dark:bg-[#180f30]"
      }`}
    >
      <div className="flex items-baseline justify-between px-1">
        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-50">{stage}</h3>
        <span className="text-xs text-slate-500 dark:text-slate-400">{opportunities.length}</span>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {opportunities.length === 0 && (
          <p className="px-1 py-2 text-center text-xs text-slate-500 dark:text-slate-400">No opportunities</p>
        )}
      </div>
    </div>
  );
