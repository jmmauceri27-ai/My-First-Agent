...
import { listDashboardsWithArea, loadDashboard } from "@/lib/dal";
...
export default async function DashboardsPage({
  ...
  const dashboards = await listDashboardsWithArea();

  const byId = new Map(dashboards.map((d) => [d.id, d]));
  const selectedArea = area ?? (id && byId.get(id)?.area) ?? dashboards[0]?.area ?? "CRM";

  const areaDashboards = dashboards.filter((d) => d.area === selectedArea);
          ...
          {areaDashboards.length > 0 && (
            <>
              ...
              {!config || config.cards.length === 0 ? (
                ...
                <DashboardViewClient key={selectedId} config={config} />
              )}
            </>
          )}
        </div>
